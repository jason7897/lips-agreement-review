# LIPS2 에이전트 (구 LIPS 협약 사업계획서 검토 도구)
...
- 처음 열면 **국가계약법 시행령(수의계약 기준금액 2천만원, 나라장터 입찰공고 의무 — 제26조/제33조)** 조항이 기본으로 자동 학습되어 있습니다. 이 기본 조항은 최초 1회만 채워지며, "전체 삭제"를 누르면 함께 삭제되고 이후 자동으로 다시 채워지지 않습니다. 법 개정 여지가 있으니 실제 적용 전 [국가법령정보센터](https://www.law.go.kr)에서 최신 원문을 한 번 더 확인하세요.
...
`lips-review.html`의 핵심 흐름(탭 전환, 지침 목록 로드, 사업비/지원요건 검토 렌더링, Q&A)을
Playwright로 자동 검증합니다. Groq 호출은 항상 가짜 응답으로 가로채고, 저장(write) 액션도
가짜로 응답해 실제 운영 시트에는 아무것도 쓰지 않습니다. 지침 목록 조회처럼 읽기 전용인
호출만 실제 라이브 백엔드를 그대로 사용합니다.
